...
class AutoMoLi(hass.Hass):  # type: ignore
    ...
    def motion_detected(
        ...
        self.lights_on(source=entity, log=log)
    ...
    def motion_event(self, event: str, data: dict[str, str], _: dict[str, Any]) -> None:
        ...
        self.lights_on(source=motion_trigger, log=log)
        self.refresh_timer()
    ...
    def lights_on(
        self, source: str = "<unknown>", force: bool = False, log: bool = False
    ) -> None:
        """Turn on the lights."""
    ...
    def turned_off(self, kwargs: dict[str, Any] | None = None) -> None:
        ...
        if overrideDelay:
            ...
            timeSinceMotion = (
                (natural_time(int(delay))).replace("\033[1m", "").replace("\033[0m", "").replace("min", " min")
            )
