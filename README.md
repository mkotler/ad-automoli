...
`activate_on_daytime_switch` | True | bool | False | Force lights to turn on/off during daytime switches and when blocking entities clear. Defaults to True when no motion sensors are defined.
`transition_on_daytime_switch` | True | bool | False | Adjust lights at a daytime's start time (instead to just setting it as active). Will not turn on lights - use activate_on_daytime_switch instead.
